...
-(UIColor *) getColor:(NSString *)pKey {
	...
	if (Nil != red) {
		...
		result = [UIColor colorWithHue:[hue floatValue] saturation:[saturation floatValue] brightness:[brightness floatValue] alpha:1.0];
	} else {
...
-(void) loadChoicePage {
	[self loadSubNib:@"ChoiceView_Phone"];
	[self loadCommon];
...
-(void) loadInputPage {
	...
	[self loadSubNib:@"InputView_Phone"];
	[self loadCommon];
...
-(void) loadTextPage {
	[self loadSubNib:@"TextView_Phone"];
	[self loadCommon];
	self.textView.text = [lua getValueForKey:@"senplayer.text"];
	self.textView.textColor = [self getColor:@"senplayer.textColor"];
}
